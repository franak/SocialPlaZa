...
//	 $('#'+components.main).css("opacity", 0);
 ...
 WAF.loadComponent ( {
		...
		//$('#'+components.main).animate({opacity:1}, 500);

        $('#overlay').remove();
//$$(components.main).show();

	//$$(components.main).show();
	//$('#'+components.main+'_matrix3').fadeIn('slow');
		...
		// UI.alert('ya!');// After successful load it must but is never called!
          }
     } );
...
functions.loguearse = function(id){
	...
	if (usuario == null)
		var error = 'No ha iniciado sesión';
...
$.getJSON( ruta, function(data) {
   ...
   var articulos = data.__ENTITIES;
   	 ...
   	 $(matrizArticulos).html( '<div class="row-fluid">'
